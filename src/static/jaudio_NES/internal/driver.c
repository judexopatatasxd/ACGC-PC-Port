...
static Acmd* Nas_SaveBufferAuto(Acmd* cmd, u16 ofs, u16 size, intptr_t startAddr);
...
static void Nas_LoadBuffer2(Acmd* cmd, s32 dst, s32 len, intptr_t src) {
    aLoadBuffer2(cmd, src, dst, len);
}

static void Nas_SaveBuffer2(Acmd* cmd, s32 src, s32 len, intptr_t dst) {
...
static void Nas_PCM8dec(Acmd* cmd, s32 flags, intptr_t state) {
...
static Acmd* Nas_SaveAuxBufferCH(Acmd* cmd, delay* del_p, s16 update_idx) {
    ...
    cmd = Nas_SaveBufferAuto(cmd, 0x6E0, param->size, (intptr_t)&del_p->left_reverb_buf[param->start_pos]);
    if (param->wrapped_size != 0) {
        cmd = Nas_SaveBufferAuto(cmd, 0x6E0 + param->size, param->wrapped_size, (intptr_t)&del_p->left_reverb_buf[0]);
    }

    aDMEMMove(cmd++, 0xDE0, 0x3A0, size);
    aSetBuffer(cmd++, 0, 0x3A0, 0x6E0, param->save_resample_num_samples * SAMPLE_SIZE);
    aResample(cmd++, del_p->resample_flags, param->save_resample_pitch, del_p->right_save_resample_buf);
    cmd = Nas_SaveBufferAuto(cmd, 0x6E0, param->size, (intptr_t)&del_p->right_reverb_buf[param->start_pos]);
    if (param->wrapped_size != 0) {
        cmd = Nas_SaveBufferAuto(cmd, 0x6E0 + param->size, param->wrapped_size, (intptr_t)&del_p->right_reverb_buf[0]);
    }
...
static Acmd* Nas_SaveBufferAuto(Acmd* cmd, u16 dmem, u16 size, intptr_t startAddr) {
    s32 startUnaligned = (s32)(startAddr & 15);
    intptr_t endAddr = startAddr + size;
...
static Acmd* Nas_SaveAuxBuffer(Acmd* cmd, delay* del_p, s16 update_idx) {
    ...
    switch (del_p->downsample_rate) {
        ...
        default:
            ...
            if (param->size != 0) {
                cmd = Nas_SaveBufferAuto(cmd, 0xC40, (u16)param->size, (intptr_t)&del_p->left_reverb_buf[param->start_pos]);
                cmd = Nas_SaveBufferAuto(cmd, 0xDE0, (u16)param->size, (intptr_t)&del_p->right_reverb_buf[param->start_pos]);
            }

            if (param->wrapped_size != 0) {
                cmd = Nas_SaveBufferAuto(cmd, 0xC40 + param->size, (u16)param->wrapped_size, (intptr_t)&del_p->left_reverb_buf[0]);
                cmd = Nas_SaveBufferAuto(cmd, 0xDE0 + param->size, (u16)param->wrapped_size, (intptr_t)&del_p->right_reverb_buf[0]);
            }
            break;
    }
...
codec_continue_and_skip:
                ...
                switch (sample->codec) {
                    ...
                    case CODEC_S8:
                        Nas_SetBuffer(cmd++, 0, sampleDataDmemAddr + sampleDataChunkAlignPad,
                                                DMEM_UNCOMPRESSED_NOTE + dmemUncompressedAddrOffset2,
                                                numSamplesToDecode * SAMPLE_SIZE);
                        Nas_PCM8dec(cmd++, flags, (intptr_t)driver->synth_params->adpcm_state);
                        break;
        ...
        if ((common->comb_filter_size != 0) && (common->comb_filter_gain != 0)) {
            ...
            if (driver->comb_filter_needs_init) {
                ...
                Nas_LoadBuffer2(cmd++, DMEM_COMB_TEMP - combFilterSize, combFilterSize, (intptr_t)combFilterState);
            }
            Nas_SaveBuffer2(cmd++, DMEM_TEMP - combFilterSize + size, combFilterSize, (intptr_t)combFilterState);
